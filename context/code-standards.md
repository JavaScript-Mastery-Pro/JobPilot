...
- Always pass accessToken to createInsforgeServer — required for RLS
...
| `profile_completed`         | User saves complete profile for first time |
| `profile_updated`           | User updates existing profile              |
| `resume_uploaded`           | User uploads resume PDF                    |
| `resume_generated`          | Base resume PDF generated from profile     |
| `profile_extracted`         | GPT-4o extracts profile from resume        |
| `linkedin_connected`        | User connects LinkedIn account             |
| `job_search_started`        | Find jobs button clicked                   |
| `job_found`                 | Each job discovered                        |
| `job_url_submitted`         | User pastes a job URL                      |
| `job_description_extracted` | GPT-4o extracts job description            |
| `job_matched`               | Job scored against profile                 |
| `cover_letter_generated`    | Cover letter created for a job             |
| `resume_tailored`           | Resume tailored for a specific job         |
| `match_score_recalculated`  | Score recalculated after editing           |
| `job_dismissed`             | User dismisses a job                       |

Event properties must always include `userId` at minimum. Include relevant context per event — jobTitle, company, score, source as appropriate.
