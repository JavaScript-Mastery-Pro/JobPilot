...
- Keep Dashboard as overview, Jobs as operations, and Profile as setup/resume
...
Public pages follow this structure:
...
Authenticated app pages live under `app/(app)/` and use `AppShell`:

```
<AppShell>
  <SidebarProvider defaultOpen>
    <Sidebar collapsible="icon" />
    <SidebarInset bg-base>
      <header> sticky top-0 h-16 bg-surface border-b border-default
      <div> max-w-7xl px-4 py-8 lg:px-6 lg:py-10
        {page content}
```

Do not add a second navbar inside authenticated pages. Page content starts with
a flat page header inside the shell content area.

### Content Width

```
Max content width:  max-w-7xl mx-auto px-6
Authenticated shell: max-w-7xl mx-auto px-4 py-8 lg:px-6 lg:py-10
Dashboard sections: overview-only within max-w-7xl
Jobs sections: operational controls, filters, and inventory within max-w-7xl
...
Used for authenticated pages only.

```
Primitive stack: SidebarProvider, Sidebar, SidebarInset, SidebarTrigger,
                 SidebarHeader, SidebarContent, SidebarMenu,
                 SidebarMenuButton, SidebarFooter, SidebarRail
Sidebar:         collapsible="icon" border-r border-default bg-surface
Header:          sticky top-0 z-30 h-16 border-b border-default bg-surface px-4
Trigger:         h-9 w-9 rounded-xl text-text-secondary
                 hover:bg-subtle hover:text-text-primary
Nav item:        h-10 rounded-xl px-3 text-text-secondary
                 hover:bg-subtle hover:text-text-primary
Active item:     border border-accent-border bg-accent-dim text-accent-text
Footer action:   h-9 rounded-xl border border-default bg-elevated px-3
```

Rules:

- Sidebar links are Dashboard, Jobs, Profile, and a disabled Settings placeholder.
- Dashboard is active only on `/dashboard`.
- Jobs is active on `/jobs` and `/jobs/[id]`.
- Profile is active only on `/profile`.
- Use `SidebarTrigger` for desktop collapse and the built-in mobile drawer.
- Close mobile navigation after route clicks.
- Keep generated shadcn sidebar classes adapted to JobPilot tokens.
...
- High-level stats.
- Current/latest run summary.
- Future analytics placement.
- No Find Jobs controls, job inventory, review queue, full live feed, or session recording.

Jobs is the operational workspace:

- Find Jobs controls.
- Active/latest run context.
- Status filters, sorting, pagination.
- All owner-scoped jobs inventory.
- Review dismiss actions and applied/failed details.

Profile is the setup and resume workspace:

- Profile fields and completion/readiness state.
- Resume evidence.
- Resume generation, preview, and download.
- Repeated evidence entries may be framed; the whole form should not be one large nested card.

Job details are drill-in pages:

- Keep job context, match breakdown, resume preview, tailoring, and manual apply together.
- Use panels only where they organize distinct job-detail tasks.
...
Used at the top of authenticated app pages.
...
Do not put page headers inside cards unless rendering a load/error fallback that
needs a contained error state.

### Page Sections

Preferred structure for large page areas after the redesign.

```
Wrapper:  no decorative background by default
Spacing:  grid gap-6, gap-5, or divided sections
Header:   compact text hierarchy; no hero-scale type
Body:     table, list, form grid, or action row
```

Use flat sections for page organization. Add borders only when they clarify a
specific tool, table, panel, repeated item, or error state.

### Cards

Used for repeated items, stats, modal content, and genuinely framed tools.
...
Never place a card inside another card. If a repeated item needs framing inside
a larger surface, the parent should be a flat section or a single panel, not a
card.

### Panels

Used for self-contained operational tools and structured data surfaces.
...
Panels may contain tables, lists, forms, or repeated item cards. Avoid stacking
panel inside panel. If the page starts feeling like boxes inside boxes, flatten
the parent into a page section.
...
Hero section:
  ...
  Desktop: lg:grid-cols-[0.92fr_1.08fr] lg:gap-14 lg:py-16
  Copy column: max-w-2xl, left aligned
  Eyebrow: inline-flex rounded-full border border-accent-border bg-accent-dim px-3 py-1 text-xs font-medium text-accent-text
  ...
  CTA buttons: standard primary/secondary button sizing — h-9 px-4 rounded-xl
  Product preview panel: bg-surface rounded-2xl border border-default shadow-elevated

How it works section:
  Section shell: border-y border-default bg-surface
  Content: max-w-7xl mx-auto px-6 py-32 lg:py-36
  Header: max-w-2xl
  Eyebrow: text-xs font-medium uppercase tracking-wide text-accent-text
  Heading: mt-3 text-3xl font-semibold text-text-primary
  Cards grid: mt-14 grid gap-6 md:grid-cols-3
  Step cards: min-h-64 bg-elevated rounded-2xl border border-default p-8
  Step number: text-sm font-medium text-accent-text
  Step title: mt-5 text-base font-medium text-text-primary
  Step body: mt-3 text-sm leading-6 text-text-secondary

Features section:
  Section shell: bg-base
  Content: max-w-7xl mx-auto px-6 py-32 lg:py-36
  Layout: grid gap-14 lg:grid-cols-[0.8fr_1.2fr]
  Cards grid: grid gap-6 sm:grid-cols-2
  Feature cards: min-h-64 bg-elevated rounded-2xl border border-default p-8
  Visual marker: h-2 w-10 rounded-full bg-accent-primary mb-5
  Feature title: text-base font-medium text-text-primary
  Feature body: mt-3 text-sm leading-6 text-text-secondary
```
